export async function sendAccessEmail({ email, code, name = '' }) {
  ...
  const origin = clean(process.env.APP_ORIGIN || 'https://atendimentoia-premiumm.vercel.app');
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    method: 'POST',
    headers: { 'api-key': apiKey, 'Content-Type': 'application/json', accept: 'application/json' },
    body: JSON.stringify({
      sender: { email: senderEmail, name: clean(process.env.BREVO_SENDER_NAME || 'OnTop Premium IA') },
  ...
  if (!response.ok) throw new Error(`Brevo respondeu ${response.status}${data?.message ? `: ${String(data.message).slice(0, 180)}` : ''}`);
  return true;
}
